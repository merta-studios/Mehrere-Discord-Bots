...
test('Challenge-Marker speichert den vollständigen Spielstand', () => {
  ...
  assert.ok(payload.startsWith('mgame::v1::'));
...
test('Vier Gewinnt erkennt waagerechte und diagonale Reihen', () => {
  ...
  const diagonalBoard = Array(42).fill(0);
  // u1 hat drei diagonal steigende Chips; der vierte fällt in Spalte 3 auf Zeile 2.
  diagonalBoard[35] = 1;
  diagonalBoard[29] = 1; diagonalBoard[36] = 2;
  diagonalBoard[23] = 1; diagonalBoard[30] = 2; diagonalBoard[37] = 2;
  diagonalBoard[31] = 2; diagonalBoard[38] = 2; diagonalBoard[24] = 2;
  let diagonal = {
    ...horizontal,
...
test('Challenge-UI pingt Gegner, zeigt Deadline und hat Annahme-/Ablehnen-Buttons', () => {
  ...
  assert.ok(text.includes(`<t:${Math.floor(state.expiresAt / 1000)}:F>`));
...
test('Custom-IDs werden strikt geparst', () => {
  ...
  assert.deepEqual(parseCustomId('mg_c4_6'), { kind: 'move', game: GAME_CONNECT4, position: 6 });
