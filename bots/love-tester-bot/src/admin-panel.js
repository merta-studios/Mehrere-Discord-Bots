/**
 * Owner-Admin-Panel Love Tester – identisch zum Birthday-/XP-Bot:
 * Serverliste mit Seiten, Server-Detail (Love-Setup-Status), Einladung,
 * Verlassen, Join-Benachrichtigung. Nur im DM mit dem Owner.
 */
...
async function renderDetailPayload(ctx, userId, guildId) {
  ...
  const setup = Boolean(entry && entry.setupComplete);
  const channelCount = entry && Array.isArray(entry.channels) ? entry.channels.length : 0;
  const lines = [
    `# ${t('apDetailTitle', 'de')}`,
    '',
  ...
  if (setup) lines.push(t('apDetailChannels', 'de', { count: channelCount }));
  ...
  switch (id) {
    ...
    case `${PANEL_PREFIX}invite`: {
      ...
      try {
        ...
        const invite = await channel.createInvite({ maxAge: 3600, maxUses: 1, reason: 'Love-Tester Admin-Panel Einladung' });
...
async function sendJoinNotice(ctx, guild) {
  if (!ctx.ownerId) {
    ctx.logger.warn('[love-tester-bot] Join-Notice übersprungen: keine Owner-ID konfiguriert.');
    return;
  }
  try {
    ...
    const container = smallContainer('👋 Neuer Love-Tester-Server!', notice);
    try {
      await dm.send(componentsV2Payload([container]));
    } catch (componentErr) {
      await dm.send({ content: `👋 Neuer Love-Tester-Server!\n\n${notice.replace(/\*\*/g, '')}` });
      ctx.logger.warn('[love-tester-bot] Join-Notice nur als Text gesendet:', componentErr.message);
    }
    ctx.logger.info(`[love-tester-bot] Join-Notice an Owner für ${guild.name} gesendet.`);
  } catch (err) {
    ctx.logger.warn('[love-tester-bot] Join-Notice fehlgeschlagen:', err.message);
  }
}
