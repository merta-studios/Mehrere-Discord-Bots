/**
 * Owner-Admin-Panel XP Bot – fast identisch zum Birthday Bot, aber XP-Details
 */
...
const PANEL_PREFIX = 'xp_ap_';
const PAGE_SIZE = 5;

function canUsePanel(ctx, interaction){
  const ownerId = String(ctx.ownerId||'').trim().replace(/^<@!?(\d+)>$/,'$1');
  const isDirectMessage = interaction.channel?.type === ChannelType.DM || (interaction.guildId==null && interaction.channel==null);
  return Boolean(ownerId) && interaction.user?.id===ownerId && isDirectMessage;
}
function deny(ctx, interaction){
  return interaction.reply(componentsV2Payload([smallContainer(null, t('apNeedDm','de'))],{ephemeral:true}));
}
function sessionOf(ctx, userId){
  if(!ctx.panelSessions.has(userId)) ctx.panelSessions.set(userId,{page:0,guildId:null,leaving:false});
...
async function renderListPayload(ctx, userId, noticePrefix=''){
  ...
  if(!servers.length){
    ...
    const select = new StringSelectMenuBuilder().setCustomId(`${PANEL_PREFIX}select`).setPlaceholder(t('apSelectPlaceholder','de')).setDisabled(true).addOptions({label:'—',value:'none'});
  ...
  const lines = pageServers.map(s=>`${s.present?'🟢':'🔴'} **${escapeMd(s.guild.name)}** · ${s.members.toLocaleString('de-DE')} Mitglieder`);
  ...
  container.addSeparatorComponents(new SeparatorBuilder().setDivider(true));
  const select = new StringSelectMenuBuilder().setCustomId(`${PANEL_PREFIX}select`).setPlaceholder(t('apSelectPlaceholder','de')).setMinValues(1).setMaxValues(1).addOptions(pageServers.map(s=>({label:s.guild.name.slice(0,90)||'Unbenannter Server',value:s.guild.id,description:`${s.members.toLocaleString('de-DE')} Mitglieder`})));
  const nav = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`${PANEL_PREFIX}prev`).setStyle(ButtonStyle.Secondary).setLabel(t('apBtnPrev','de')).setDisabled(page===0),
    new ButtonBuilder().setCustomId(`${PANEL_PREFIX}refresh`).setStyle(ButtonStyle.Primary).setLabel(t('apBtnRefresh','de')),
    new ButtonBuilder().setCustomId(`${PANEL_PREFIX}next`).setStyle(ButtonStyle.Secondary).setLabel(t('apBtnNext','de')).setDisabled(page>=totalPages-1)
  );
  container.addActionRowComponents(new ActionRowBuilder().addComponents(select), nav);
  return componentsV2Payload([container]);
}

function escapeMd(text){ return String(text).replace(/([\\*_`|~])/g,'\\$1'); }

async function renderDetailPayload(ctx, userId, guildId){
  const session = sessionOf(ctx,userId);
  const guild = ctx.client.guilds.cache.get(guildId);
  if(!guild){ session.guildId=null; return renderListPayload(ctx,userId); }
  const entry = ctx.store.getGuild(guildId);
  const setup = !!entry;
  const usersCount = setup ? ctx.store.getUsersForGuild(guildId).length : 0;
  const lines=[
    `# ${t('apDetailTitle','de')}`,
    '',
  ...
  if(setup) lines.push(t('apDetailXpUsers','de',{count:usersCount}));
  if(session.leaving) lines.push('',`⚠️ ${t('apLeaveAsk','de',{name:guild.name})}`);
  else if(session.inviteUrl) lines.push('',`**${t('apInviteSent','de')}**`, t('apInviteLink','de',{url:session.inviteUrl}));
  const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(lines.join('\n')));
  container.addSeparatorComponents(new SeparatorBuilder().setDivider(true));
  const buttons=[];
  if(session.leaving){
    buttons.push(
      new ButtonBuilder().setCustomId(`${PANEL_PREFIX}leave_confirm`).setStyle(ButtonStyle.Danger).setLabel(t('apBtnLeaveConfirm','de')),
      new ButtonBuilder().setCustomId(`${PANEL_PREFIX}leave_cancel`).setStyle(ButtonStyle.Secondary).setLabel(t('apBtnLeaveCancel','de'))
    );
  } else {
    buttons.push(
      new ButtonBuilder().setCustomId(`${PANEL_PREFIX}back`).setStyle(ButtonStyle.Secondary).setLabel(t('apBtnBack','de')),
      new ButtonBuilder().setCustomId(`${PANEL_PREFIX}invite`).setStyle(ButtonStyle.Primary).setLabel(t('apBtnInvite','de')),
      new ButtonBuilder().setCustomId(`${PANEL_PREFIX}leave`).setStyle(ButtonStyle.Danger).setLabel(t('apBtnLeave','de'))
    );
  }
...
async function handlePanelButton(ctx, interaction){
  ...
  switch(id){
    ...
    case `${PANEL_PREFIX}back`: {
      await interaction.deferUpdate();
      session.guildId=null; session.leaving=false;
      return interaction.editReply(await renderListPayload(ctx,userId));
    }
    case `${PANEL_PREFIX}invite`: {
      await interaction.deferUpdate();
      const guild = ctx.client.guilds.cache.get(session.guildId);
      if(!guild){ session.guildId=null; return interaction.editReply(await renderListPayload(ctx,userId)); }
      try{
        const channels = await guild.channels.fetch().catch(()=>guild.channels.cache);
        const channel = channels.find(c=>c.type===ChannelType.GuildText && c.permissionsFor(ctx.client.user)?.has(PermissionFlagsBits.CreateInstantInvite)) || guild.systemChannel;
        if(!channel) throw new Error('Kein passender Channel gefunden.');
        const invite = await channel.createInvite({maxAge:3600,maxUses:1,reason:'XP Admin-Panel Einladung'});
        session.inviteUrl = invite.url;
      } catch(err){ session.inviteError = t('apInviteFailed','de',{error:err.message}); }
...
async function handlePanelSelect(ctx, interaction){
  ...
  const session = sessionOf(ctx, interaction.user.id);
  session.guildId=guildId; session.leaving=false; session.inviteUrl=null;
  await interaction.deferUpdate();
...
module.exports = { PANEL_PREFIX, openPanel, handlePanelButton, handlePanelSelect, sendJoinNotice };
