...
> **Leveln durch Chatten!** 3 XP pro echtem Wort (max 30 XP) + **15 XP für Bilder, Videos & Sprachnachrichten**, 30s Cooldown gegen Spam, Voice 25 XP/Minute, täglicher **10%-Schwund um Mitternacht** (wer inaktiv bleibt: **+5 Prozentpunkte je Inaktiv-Tag** – 10 %, 15 %, 20 % …), Leaderboard Top15 **zuverlässig stündlich + bei Level-Ups**, **geplante Bonus-Geschenke (20–40 XP, 2–4×/Tag) mit „Einsammeln“-Button im Haupt-Chat**, **Level-Belohnungsrollen per Formular** – alles in 10 Sprachen, modernen Components V2 & mit Turso so sparsam wie möglich!
...
| **Täglicher Schwund** | **Jeden Tag um 0 Uhr** (Zeitzone der Server-Sprache) verliert jeder **10 %** der für sein nächstes Level nötigen XP. **Inaktivitäts-Streak:** Hat ein Nutzer in den letzten 24 h **keine XP verdient**, steigt der Anteil pro weiterem inaktivem Tag um **+5 Prozentpunkte** (1. inaktiver Tag = 10 %, dann 15 %, 20 %, 25 % …). Sobald wieder XP verdient wird, fällt der Satz auf **10 %** zurück. Wer auf Level 1 mit 0 XP steht, verliert nichts. Fällt XP unter 0, **verlierst du ein Level** und der Restbetrag wird korrekt vom vorherigen Level abgezogen. `/rank` zeigt als Stichpunkt, **wie viel XP du heute um 0 Uhr verlieren wirst**. ⚠️ Hinweis steht auch im Leaderboard. |
| **Bonus-Geschenke** | **Geplant & zeitgesteuert**: Pro Server werden täglich **2–4 feste Termine** ausgelöst (deterministisch aus Server-ID + Datum – **jeder Server hat andere, aber stabile Zeiten**, kein Neuwürfeln bei Neustarts). Alle Termine liegen **mindestens 1 h auseinander** und nur **zwischen 06:00 und 00:30 Uhr Ortszeit** (kein mitten in der Nacht). Zum Termin erscheint ein Bonus-Container mit **zufällig 20–40 XP**, „🎁 Einsammeln“-Button – **der erste Klick gewinnt**. Der Drop ist **1 Stunde gültig** und verfällt danach unwiderruflich (Button wird deaktiviert, die Nachricht pingt ggf. den Gewinner). Wer einsammelt, gilt wieder als aktiv – **sein täglicher XP-Schwund fällt auf 10 % zurück**. Ohne Leaderboard-Setup gibt es keine Boni. |
...
| **Nickname** | Sofort nach Level Up/Down: `[Lvl {LVL} 🥇] Anzeigename` – nur die **Top 3** bekommen eine Medaille (`🥇🥈🥉`) in den Nicknamen. **Rang-Verschiebungen werden zuverlässig nachgezogen:** Bei jedem Level-Up/Down werden die Top 5 + der betroffene Nutzer geprüft, damit z.B. ein neuer Platz 2 sofort 🥈 im Anzeigenamen bekommt (auch wer aus den Top 3 fällt, verliert die Medaille). Selbst XP-only-Überholer (gleiches Level) werden per 2-Minuten-Check abgefangen. Wird bestehender Tag überschrieben, bei >32 Zeichen wird Anzeigename rechts gekürzt. Kann Bot nicht umbenennen → Ping im Haupt-Chat: „Ich brauche Rolle über allen anderen!“ (Ausnahme: der **Server-Owner** bekommt diesen Hinweis nicht, da Discord es nie erlaubt, ihn umzubenennen). |
| **Leaderboard** | **Zuverlässig stündlich** aktualisiert (zeitbasiert, nicht tickbasiert – klappt auch nach Prozess-Pausen/Restarts; sofort beim Start und **immer nach dem 0-Uhr-Schwund**, damit der Zeitstempel stimmt) **plus bei jedem Level-Up/Down**, aber frühestens alle **10 Minuten** (Throttle). Mit **letzter Aktualisierung + Zeitzone**, Update-Hinweis & **kurzer Schwund-Warnung**. Schlägt das Editieren fehl, wird die alte Nachricht gelöscht und neu gesendet (kein veralteter Zeitstempel bleibt stehen). Marker `xp_leader::v1::` für Self-Healing (Scan jetzt 100 Nachrichten tief). |
