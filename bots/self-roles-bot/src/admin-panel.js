/**
 * Owner-Admin-Panel (/adminpanel) des Self-Roles-Bots – nur im Privatchat
 ...
 * - Server-Detail: Owner-Mention, Name, Mitglieder, Self-Roles-Status
 *   (wie viele Nachrichten, wie viele Rollen).
...
async function renderDetailPayload(ctx, userId, guildId) {
  ...
  const roles = ctx.store.totalRoles(guildId);
  ...
  if (setup) {
    lines.push(t('apDetailMessages', 'de', { count: messages, roles }));
  }
...
async function sendJoinNotice(ctx, guild) {
  if (!ctx.ownerId) {
    ctx.logger.warn('[self-roles-bot] Join-Notice übersprungen: keine Owner-ID konfiguriert.');
    return;
  }
  try {
    ...
    try {
      ...
      ctx.logger.warn('[self-roles-bot] Join-Notice nur als Text gesendet:', componentErr.message);
    }
    ctx.logger.info(`[self-roles-bot] Join-Notice an Owner für ${guild.name} gesendet.`);
  } catch (err) {
    ctx.logger.warn('[self-roles-bot] Join-Notice konnte nicht gesendet werden:', err.message);
  }
}
